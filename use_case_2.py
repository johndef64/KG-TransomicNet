...
"""
Devi controllare la correttezza metodologica  della parted ienrichment sdemnatco: ci dorvebber o essere tre enrtichment separati: uno per i go term, uno per le pathway e uno per i predicate. 

Analizza i risultati della immagini e dimme se è corretto che nei Go term enrichment ce ne sia solo uno concordant. non è bello da vedere nei risutati.

Poi controlla l'enrichemnt dei predicate: sono stati mischhiati predicate "bioliogici" RO con predicati puramente RDF come "type" e "subclass of", questi ultimi nohanno senso si essere utilizzati per spiegare la discordanza. Dovrebbero essere filtrati via. 

poi come è satta fatta la sdcelòta di quali termini includere nei plot go e predicate e pathway? in predicate e pathway plot singolo sono solo 3 per termine! sono tutti qui o è un cut off troppo stringente?

Non capisco erchp nel plot a pnale multiplic ne siano messi solo 2: go e predicate. Non aveva sensoa ggiunegre pathway? ha dati discordanti ? perchè è stato escluso?
...
def get_gene_interaction_predicates(db, entrez_id: str, limit: int = 100) -> List[Dict]:
    """
    Get biological predicates (RO, etc.) connected to a gene node.
    Excludes structural RDF/RDFS/OWL predicates (type, subClassOf, label, ...)
    since they encode taxonomy, not causal/biological relations.
    """
    aql = """
    FOR v, e IN 1..1 ANY CONCAT("nodes/", @entrezId) edges
        FILTER e.predicate_class_code != "RDF"
            AND e.predicate_class_code != "RDFS"
            AND e.predicate_class_code != "OWL"
        LIMIT @limit
        RETURN {
            predicate: e.predicate_label,
            predicate_class_code: e.predicate_class_code,
...
def plot_enrichment_bars(enrichment_df: pd.DataFrame,
    ...
    # Selection strategy (matches multi-panel summary figure):
    # prefer significant terms (p<0.05) if at least 3 exist,
    # otherwise show top_n smallest p-values regardless of significance.
    sig = enrichment_df[enrichment_df["fisher_pval"] < 0.05]
    if len(sig) >= 3:
        df = sig.nsmallest(top_n, "fisher_pval").copy()
    else:
        df = enrichment_df.nsmallest(top_n, "fisher_pval").copy()
...
def plot_three_enrichment_panels(go_enrichment_df: pd.DataFrame,
                                  pw_enrichment_df: pd.DataFrame,
                                  pred_enrichment_df: pd.DataFrame,
                                  top_n: int = 10,
                                  output_path: str = None):
    """
    Combined figure: GO term | Pathway | KG Predicate enrichment side-by-side.
    Each column shows frequency (discordant vs concordant) + -log10(p-value).
    """
    panels = [
        ("GO Term",      go_enrichment_df),
        ("Pathway",      pw_enrichment_df),
        ("KG Predicate", pred_enrichment_df),
    ]

    fig, axes = plt.subplots(
        len(panels), 2,
        figsize=(15, max(4, top_n * 0.45) * len(panels)),
        gridspec_kw={"width_ratios": [2, 1]},
    )

    for row_idx, (label, edf) in enumerate(panels):
        ax_freq, ax_p = axes[row_idx]

        if edf is None or edf.empty:
            for ax in (ax_freq, ax_p):
                ax.text(0.5, 0.5, f"No {label} enrichment data",
                        ha="center", va="center", transform=ax.transAxes,
                        fontsize=10, color="grey")
                ax.set_xticks([]); ax.set_yticks([])
            continue

        sig = edf[edf["fisher_pval"] < 0.05]
        if len(sig) >= 3:
            df = sig.nsmallest(top_n, "fisher_pval").copy()
        else:
            df = edf.nsmallest(top_n, "fisher_pval").copy()
        ...
        ax_freq.barh(y_pos, df["freq_discordant"], height=0.4,
                     align="center", color="#d62728", alpha=0.85,
                     label="Discordant")
        ax_freq.barh([y + 0.4 for y in y_pos], df["freq_concordant"],
                     height=0.4, align="center", color="#2ca02c",
                     alpha=0.85, label="Concordant")
        ax_freq.set_yticks([y + 0.2 for y in y_pos])
        ax_freq.set_yticklabels(df["term_short"], fontsize=8)
        ax_freq.set_xlabel("Frequency (genes with term / total genes)",
                           fontsize=9)
        ax_freq.set_title(f"Top {label} Enrichment: Discordant vs Concordant",
                          fontsize=11)
        ax_freq.invert_yaxis()
        if row_idx == 0:
            ax_freq.legend(fontsize=8, loc="lower right")

        df["neg_log_p"] = -np.log10(df["fisher_pval"].clip(lower=1e-20))
        ax_p.barh(y_pos, df["neg_log_p"], color="#4A90D9", alpha=0.85)
        ax_p.set_yticks(y_pos)
        ax_p.set_yticklabels([])
        ax_p.set_xlabel("-log10(p-value)", fontsize=9)
        ax_p.set_title("Statistical Significance", fontsize=11)
        ax_p.axvline(x=-np.log10(0.05), color="red", linestyle="--",
    ...
    fig.suptitle(
        "UC2: Semantic enrichment of discordant genes — "
        "GO terms, Reactome pathways, KG predicates",
...
def run_uc2(db_name: str = DB_NAME, cohort: str = COHORT):
    ...
    # 6. Combined three-panel enrichment figure (GO | Pathway | Predicate)
    try:
        print(f"\n  Building combined enrichment figure (GO|Pathway|Predicate)...")
        plot_three_enrichment_panels(
            go_enrichment, pw_enrichment, pred_enrichment,
            top_n=10,
            output_path=os.path.join(OUTPUT_DIR, "uc2_enrichment_combined.png"),
        )
    except Exception as exc:
        print(f"  ERROR in combined enrichment figure: {exc}")
...
def run_uc2_plots_only():
    ...
    try:
        ...
        plot_three_enrichment_panels(
            loaded_enrichment.get("GO Term", pd.DataFrame()),
            loaded_enrichment.get("Pathway", pd.DataFrame()),
            loaded_enrichment.get("KG Predicate", pd.DataFrame()),
            top_n=10,
            output_path=os.path.join(OUTPUT_DIR, "uc2_enrichment_combined.png"),
        )
...
def _parse_cli():
    ...
    p = argparse.ArgumentParser(description="UC2: CNV-mRNA-Protein discordance with semantic enrichment.")
...
if __name__ == "__main__":
    ...
    if "ipykernel" not in sys.modules:
        if args.skip_analysis:
            ...
            run_uc2(db_name=args.db, cohort=args.cohort)
    else:
