...
def traverse_phenotype_to_genes(db, seed_key: str,
    ...
    for disease_batch, batch_label in [(breast_mondo, "breast-specific"),
        ...
        for r in d2g_results:
            ...
            if not eid:
                continue
            # Always record disease->gene edge (KG fact), regardless of whether
            # the gene was already discovered via hop-1 or another disease.
            # Otherwise disease nodes appear orphan in the subgraph plot.
            subgraph_edges.append((r["disease_key"], r["gene_key"], r["pred"]))
            if eid not in genes and eid not in hop2_genes:
...
# -----------------------------------------------------------------------------
# Paper-ready styling helpers (Bioinformatics journal: Arial/Helvetica, sans-serif,
# 7-9pt, max width 174mm = 6.85in single col / 7.08in double col).
# -----------------------------------------------------------------------------
_BIOINFORMATICS_RC = {
...
def _build_semantic_graph(traversal_result, seed_node,
                          mean_expression, max_genes, max_others):
    """Build DiGraph + return (G, node_types, type_colors, expr_norm, expr_cmap)."""
    G = nx.DiGraph()
    ...
    # Rank genes by expression desc (else by hop dist asc)
    gene_items = list(genes.items())
    if mean_expression is not None:
        gene_items.sort(
            key=lambda kv: (
                -float(mean_expression.get(kv[0], 0) or 0),
                kv[1].get("hop_distance", 99),
            )
        )
    gene_items = gene_items[:max_genes]
    for eid, info in gene_items:
        short = (info["label"].split(" (")[0] if info["label"] else eid)[:18]
        mval = float(mean_expression[eid]) if (mean_expression is not None
                                               and eid in mean_expression.index) else 0
        G.add_node(info["kg_key"], label=short, node_type="gene",
                   mean_expr=mval, entrez_id=eid)

    # Genes already added → keys_in_genes used to filter connected proteins/pathways
    selected_gene_keys = {info["kg_key"] for _, info in gene_items}

    # Build adjacency from full traversal_edges to know who connects to selected genes
    edges_full = traversal_result["subgraph_edges"]
    nbr_count = defaultdict(int)  # node_key -> # edges to selected genes
    for src, tgt, _pred in edges_full:
        if src in selected_gene_keys:
            nbr_count[tgt] += 1
        if tgt in selected_gene_keys:
            nbr_count[src] += 1

    # Diseases: phenotype-anchored (always connected to seed). Rank by
    # connectivity to the selected gene set (desc) so the displayed diseases
    # are those that contribute the most to the trans-omic story.
    # Breast-specific diseases get a small bonus to keep clinical context.
    disease_candidates = []
    for dk, dinfo in diseases.items():
        score = nbr_count.get(dk, 0)
        if dinfo.get("is_breast_specific"):
            score += 0.5  # tiebreaker, not dominant
        disease_candidates.append((dk, dinfo, score))
    disease_candidates.sort(key=lambda x: -x[2])
    for dk, dinfo, _score in disease_candidates[:max_others]:
        G.add_node(dk, label=(dinfo["label"] or dk)[:24],
                   node_type="disease", mean_expr=0)

    # Pathways: ONLY include if connected to ≥1 selected gene; rank by # connections desc
    pathway_candidates = [
        (pk, pinfo, nbr_count.get(pk, 0))
        for pk, pinfo in pathways.items()
        if nbr_count.get(pk, 0) > 0
    ]
    pathway_candidates.sort(key=lambda x: -x[2])
    for pk, pinfo, _deg in pathway_candidates[:max_others]:
        G.add_node(pk, label=(pinfo["label"] or pk)[:24],
                   node_type="pathway", mean_expr=0)

    # Proteins: ONLY include if connected to ≥1 selected gene; rank by # connections desc
    protein_candidates = [
        (prk, prinfo, nbr_count.get(prk, 0))
        for prk, prinfo in proteins.items()
        if nbr_count.get(prk, 0) > 0
    ]
    protein_candidates.sort(key=lambda x: -x[2])
    for prk, prinfo, _deg in protein_candidates[:max_others]:
        G.add_node(prk, label=(prinfo["label"] or prk)[:18],
                   node_type="protein", mean_expr=0)

    keys_in = set(G.nodes())
    for src, tgt, pred in edges_full:
        if src in keys_in and tgt in keys_in:
    ...
    if len(G.edges()) == 0:
        ...
        for eid, info in genes.items():
            gk = info["kg_key"]
            if gk in keys_in and info["hop_distance"] <= 2:
    ...
    if mean_expression is not None:
        vals = [v for n, v in nx.get_node_attributes(G, "mean_expr").items()
                if node_types.get(n) == "gene" and v and v > 0]
        if vals:
            vmin, vmax = np.percentile(vals, [5, 95])
            expr_norm = Normalize(vmin=max(vmin, 0.01), vmax=max(vmax, 1))

    return G, node_types, type_colors, expr_norm, expr_cmap


def _node_color_array(G, node_types, type_colors, expr_norm, expr_cmap):
    colors = [type_colors.get(node_types.get(n, "gene"), "#CCCCCC")
              for n in G.nodes()]
    if expr_norm is not None:
        mexpr = nx.get_node_attributes(G, "mean_expr")
        for i, n in enumerate(G.nodes()):
            if node_types.get(n) == "gene" and mexpr.get(n, 0) > 0:
                colors[i] = expr_cmap(0.30 + 0.65 * expr_norm(mexpr[n]))
    return colors


def _layout_sectorial(G, node_types, seed_keys):
    """Sectorial radial: each non-phenotype type gets an angular sector.
    Reduces edge crossings vs. concentric rings because edges within a sector
    are short and edges across sectors travel through the centre.
    """
    sectors = {
        "disease": (np.deg2rad(60),  np.deg2rad(120)),   # top wedge
        "pathway": (np.deg2rad(150), np.deg2rad(210)),   # left wedge
        "protein": (np.deg2rad(240), np.deg2rad(300)),   # bottom wedge
        "gene":    (np.deg2rad(-30), np.deg2rad(60)),    # right wedge
    }
    radii = {"disease": 0.55, "pathway": 0.85, "protein": 0.85, "gene": 1.0}
    degrees = dict(G.degree())
    pos = {sk: np.array([0.0, 0.0]) for sk in seed_keys}
    for nt, (a0, a1) in sectors.items():
        members = [n for n, t in node_types.items() if t == nt]
        if not members:
            continue
        members.sort(key=lambda n: -degrees.get(n, 0))
        # Use 1-2 sub-rings for large sets (gene)
        n_m = len(members)
        max_per_ring = 60 if nt == "gene" else 40
        n_rings = int(np.ceil(n_m / max_per_ring))
        for i, n in enumerate(members):
            ring_idx = i % n_rings
            slot = i // n_rings
            slots_in_ring = int(np.ceil(n_m / n_rings))
            frac = (slot + 0.5) / max(slots_in_ring, 1)
            theta = a0 + frac * (a1 - a0)
            r_base = radii[nt]
            r = r_base * (1.0 + 0.18 * ring_idx)
            pos[n] = np.array([r * np.cos(theta), r * np.sin(theta)])
    return pos


def _layout_hierarchical(G, node_types, seed_keys):
    """Hierarchical bipartite: phenotype -> disease -> gene -> protein/pathway,
    arranged top-down on parallel horizontal bands."""
    bands = {
        "phenotype":  0.80,
        "disease":    0.45,
        "gene":       0.00,
    ...
    for n, t in node_types.items():
        members_by_band.setdefault(t, []).append(n)
    pos = {}
    for t, members in members_by_band.items():
        members.sort(key=lambda n: -degrees.get(n, 0))
        n_m = len(members)
        if n_m == 0:
            continue
        if t == "phenotype":
            for n in members:
                pos[n] = np.array([0.0, bands[t]])
            continue
        # spread x in [-1,1]
        if t in ("protein", "pathway"):
            # split the bottom band: protein on left, pathway on right
            x0, x1 = (-1.0, -0.05) if t == "protein" else (0.05, 1.0)
        elif t == "gene":
            # wider spread so gene nodes don't overlap
            x0, x1 = -1.2, 1.2
        else:
            x0, x1 = -1.0, 1.0
        if n_m == 1:
            ...
            xs = np.linspace(x0, x1, n_m)
        for n, x in zip(members, xs):
            pos[n] = np.array([float(x), bands[t]])
    return pos


def _layout_concentric_rings(G, node_types, seed_keys):
    """Improved concentric rings: order = phenotype, disease, gene, protein, pathway.
    Sort each ring by degree; radii sized so circumference >= n_nodes * min_arc."""
    ring_order = ["disease", "gene", "protein", "pathway"]
    degrees = dict(G.degree())
    counts = {t: max(sum(1 for _ in (n for n, tt in node_types.items() if tt == t)), 1)
              for t in ring_order}
    min_arc = 0.50
    radii = {}
    last_r = 0.0
    floors = {"disease": 0.45, "gene": 0.85, "protein": 1.20, "pathway": 1.45}
    for t in ring_order:
        r_need = counts[t] * min_arc / (2 * np.pi)
        r = max(r_need, floors[t], last_r + 0.30)
        radii[t] = r
        last_r = r
    pos = {sk: np.array([0.0, 0.0]) for sk in seed_keys}
    for t in ring_order:
        members = [n for n, tt in node_types.items() if tt == t]
        if not members:
            continue
        members.sort(key=lambda n: -degrees.get(n, 0))
        n_m = len(members)
        phase = {"disease": 0, "gene": np.pi / max(n_m, 1),
                 "protein": 0, "pathway": np.pi / max(n_m, 1)}.get(t, 0)
        r = radii[t]
        for i, n in enumerate(members):
            theta = 2 * np.pi * i / n_m + phase
            pos[n] = np.array([r * np.cos(theta), r * np.sin(theta) * 0.85])
    return pos
...
def plot_semantic_subgraph(traversal_result: Dict,
                           ...
                           max_nodes: int = 120,
                           layout: str = "sectorial",
                           max_genes: int = 60,
                           max_others: int = 12,
                           paper_ready: bool = True):
    """Paper-ready semantic subgraph (Bioinformatics specs).

    layout:
      - "sectorial"    : angular sectors per node type (default, lowest crossings)
      - "hierarchical" : top-down bands (phenotype -> disease -> gene -> protein/pathway)
      - "rings"        : improved concentric rings (phenotype -> disease -> gene -> protein -> pathway)
      - "spring"/"circular" : legacy fallbacks
    """
    G, node_types, type_colors, expr_norm, expr_cmap = _build_semantic_graph(
        traversal_result, seed_node, mean_expression, max_genes, max_others)
    ...
    # ── Layout dispatch ───────────────────────────────────────────────────
    pos: Dict = {}
    layout_norm = (layout or "sectorial").lower()
    if layout_norm in ("sectorial", "sector", "wedge"):
        pos = _layout_sectorial(G, node_types, seed_keys)
    elif layout_norm in ("hierarchical", "hier", "bipartite", "layered"):
        pos = _layout_hierarchical(G, node_types, seed_keys)
    elif layout_norm in ("rings", "concentric"):
        pos = _layout_concentric_rings(G, node_types, seed_keys)
    elif layout_norm == "circular":
        pos = _layout_concentric_rings(G, node_types, seed_keys)  # alias
    else:
        # legacy spring fallback (unchanged behaviour)
        pos = _layout_concentric_rings(G, node_types, seed_keys)

    # ── Paper-ready figure ────────────────────────────────────────────────
    rc_ctx = plt.rc_context(_BIOINFORMATICS_RC) if paper_ready else plt.rc_context({})
    with rc_ctx:
        # Bioinformatics double-column = 7.08 in. Hierarchical = wider/shorter.
        if layout_norm.startswith("hier"):
            figsize = (14.0, 6.5)  # wide for gene row + extra height for 45° labels
        else:
            figsize = (7.08, 6.0)
        fig, ax = plt.subplots(figsize=figsize)

        # Node sizes (compact, scaled for small figure)
        node_sizes = []
        for n in G.nodes():
        ...
        node_colors = _node_color_array(G, node_types, type_colors,
                                        expr_norm, expr_cmap)

        # Edges: classify cross-type vs intra-type for visual distinction
        cross_edges, intra_edges = [], []
        for u, v in G.edges():
        ...
        if intra_edges:
            nx.draw_networkx_edges(G, pos, ax=ax, edgelist=intra_edges,
                                   edge_color="#B0B7BC", alpha=0.35,
                                   arrows=False, width=0.4)
        if cross_edges:
            nx.draw_networkx_edges(G, pos, ax=ax, edgelist=cross_edges,
                                   edge_color="#5D6D7E", alpha=0.55,
                                   arrows=False, width=0.55)
        ...
        # Compute global span for offset scaling
        xs_all = np.array([p[0] for p in pos.values()])
        ys_all = np.array([p[1] for p in pos.values()])
        span = max(np.ptp(xs_all), np.ptp(ys_all), 1e-3)
        off = 0.025 * span
        ...
        for n in G.nodes():
            ...
            fontsize = 5.5 if nt == "gene" else 6.5
            fontweight = "bold"

            if layout_norm.startswith("hier"):
                # All bands: 45° rotation, anchored below the row for top bands
                # (phenotype/disease/gene) and above for bottom band (protein/pathway)
                if y >= 0:
                    tx, ty = x, y - off * 0.4
                    ha, va, rot = "right", "top", 45
                else:
                    tx, ty = x, y + off * 0.4
                    ha, va, rot = "left", "bottom", 45
            ...
            # Sectorial / rings: rotate label tangentially so it reads along
            # the radial direction (radial-outward orientation, no overlap).
            r = np.hypot(x, y)
            if r < 1e-6:
                dx, dy = 0.0, off
                rot = 0
                ...
                dx, dy = x / r * off, y / r * off
                # Angle of the radial direction (degrees)
                ang = np.degrees(np.arctan2(y, x))
                # Keep text upright: flip if pointing leftwards
                if -90 <= ang <= 90:
                    rot = ang
                    ha = "left"
                else:
        ...
        # Sector labels (sectorial only)
        if layout_norm.startswith("sector"):
            sector_anchor = {
                "Diseases":  (0.0,  1.18),
                "Pathways":  (-1.18, 0.0),
                "Proteins":  (0.0, -1.18),
            ...
            for lbl, (x, y) in sector_anchor.items():
                ax.text(x, y, lbl, ha="center", va="center",
                        fontsize=7, fontweight="bold",
                        color="#566573", style="italic", zorder=1)
        ...
        if expr_norm is not None:
            ...
            sm.set_array([])
            cax = fig.add_axes([0.90, 0.15, 0.012, 0.22])
            cb = fig.colorbar(sm, cax=cax)
            cb.set_label("Gene mean expr. (TPM)", fontsize=6.5,
                         fontweight="bold")
            cb.ax.tick_params(labelsize=6)
            cax.text(0.5, -0.10, "grey = no data",
                     transform=cax.transAxes, ha="center", va="top",
                     fontsize=5.5, color="#7F8C8D")

        ax.set_title(title, fontsize=9, fontweight="bold", pad=6, color="black")

        # Frame fitting
        pad_x = np.ptp(xs_all) * 0.10 if len(xs_all) else 0.2
        pad_y = np.ptp(ys_all) * 0.12 if len(ys_all) else 0.2
        ax.set_xlim(xs_all.min() - pad_x, xs_all.max() + pad_x)
        ax.set_ylim(ys_all.min() - pad_y, ys_all.max() + pad_y)
        ax.set_aspect("equal" if not layout_norm.startswith("hier") else "auto")
        ax.axis("off")
        plt.tight_layout(rect=(0, 0, 0.88, 1))

        if output_path:
            base, ext = os.path.splitext(output_path)
            # Always emit PNG @600dpi + PDF (vector) for paper submission
...
def run_uc3(db_name: str = DB_NAME, cohort: str = COHORT,
    ...
    # 7a. Semantic subgraph (paper-ready, 3 layouts)
    print("  [7a] Semantic subgraph (paper-ready, 3 layouts)...")
    _seed_lbl = seed_node.get('label', 'Phenotype')[:40]
    for _lname, _lkey in [("sectorial", "A"), ("hierarchical", "B"), ("rings", "C")]:
        plot_semantic_subgraph(
            traversal, seed_node,
            mean_expression=mean_expression,
            title=f"({_lkey}) UC3: Semantic Subgraph from {_seed_lbl}",
            output_path=os.path.join(
                OUTPUT_DIR, f"uc3_semantic_subgraph_{_lname}.png"),
            layout=_lname,
        )
...
def _parse_cli():
    ...
    p = argparse.ArgumentParser(description="UC3: Phenotype-anchored multi-layer projection.")
    p.add_argument("--db", default=DB_NAME,
                   help=f"ArangoDB database name (default: {DB_NAME}).")
    p.add_argument("--cohort", default=COHORT,
                   help=f"TCGA cohort (default: {COHORT}).")
    p.add_argument("--host", default=arangodb_utils.arangodb_hosts,
                   help=f"ArangoDB host URL (default: {arangodb_utils.arangodb_hosts}).")
    p.add_argument("--user", default=arangodb_utils.arangodb_user,
                   help=f"ArangoDB user (default: {arangodb_utils.arangodb_user}).")
    p.add_argument("--password", default=arangodb_utils.arangodb_password,
                   help="ArangoDB password (default: same as the project default).")
    p.add_argument("--skip-analysis", action="store_true",
                   help="Regenerate plots from cached CSVs (no DB connection).")
    p.add_argument("--pdf", action="store_true",
                   help="Also export figures to PDF.")
    p.add_argument("--display-plot", action="store_true",
                   help="Show figures interactively in addition to saving them "
                        "(default: non-interactive Agg backend).")
...
if __name__ == "__main__":
    ...
    DISPLAY_PLOTS = args.display_plot
    if not DISPLAY_PLOTS:
    ...
    if "ipykernel" not in sys.modules:
        if args.skip_analysis:
            ...
            run_uc3(db_name=args.db, cohort=args.cohort)
    else:
        ...
        if args.skip_analysis:
            ...
            traversal, omic_dfs, summary_df = run_uc3(
                db_name=args.db, cohort=args.cohort,
            )
