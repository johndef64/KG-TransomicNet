...
# Hop-distance analysis: seed genes used in the published figure.
# These are the 15 Entrez IDs originally drawn from `all_connected_genes`
# during the first end-to-end run that produced the paper figure. They are
# pinned here because Python `set` iteration order is non-deterministic across
# processes (PYTHONHASHSEED randomization), so resampling on a different DB or
# in a different process would yield a different — equally valid but
# non-comparable — figure.
#
# Pass --random-seeds on the command line to pick 15 seeds at runtime instead
# (useful for robustness checks).
SEED_GENES_HOP_PUBLISHED = [
    "1082", "1902", "2316", "285093", "3354", "353091", "390073",
    "51203", "51393", "5525", "5566", "5732", "6159", "6546", "9351",
]
...
def run_uc1(db_name: str = DB_NAME, cohort: str = COHORT,
    ...
    ----------
    random_seeds : bool
        If False (default), use SEED_GENES_HOP_PUBLISHED for the hop-distance
        analysis to reproduce the figure shipped with the paper. If True,
        pick 15 seeds at runtime from the connected-gene set (subject to
        Python set-iteration order, hence non-deterministic between processes).
    """
    print("=" * 70)
    ...
    if random_seeds:
        # Sample 30 seeds at runtime; we then take :15 below.
        # Sorting first removes the Python-set iteration ambiguity, but the
        # outcome still depends on which genes are connected on the current DB.
        seed_candidates = sorted(all_connected_genes)[:30]
        print(f"  [--random-seeds] picked {len(seed_candidates)} candidate seeds from {len(all_connected_genes)} connected genes")
    else:
        # Use the published seed list to reproduce the paper figure exactly.
        present = [g for g in SEED_GENES_HOP_PUBLISHED if g in all_connected_genes]
        missing = [g for g in SEED_GENES_HOP_PUBLISHED if g not in all_connected_genes]
        if missing:
            print(f"  [warn] {len(missing)}/{len(SEED_GENES_HOP_PUBLISHED)} published seeds not connected on this DB: {missing}")
        seed_candidates = present
        print(f"  Using {len(seed_candidates)}/{len(SEED_GENES_HOP_PUBLISHED)} published seed genes (pass --random-seeds to resample).")
    hop_df = pd.DataFrame()
    if seed_candidates:
...
def _parse_cli():
    ...
    p = argparse.ArgumentParser(description="UC1: KG semantic relations vs cross-layer correlation.")
    ...
    p.add_argument("--random-seeds", action="store_true",
                   help="Resample hop-distance seed genes at runtime "
                        "(default: use pinned SEED_GENES_HOP_PUBLISHED).")
    # Filter out unknown args (e.g. Jupyter kernel arguments) so we don't crash interactively
    args, _ = p.parse_known_args()
    # Override module-level defaults so setup_arangodb_connection() honours the CLI overrides
...
if __name__ == "__main__":
    ...
    if "ipykernel" not in sys.modules:
        if args.skip_analysis:
            ...
            run_uc1(db_name=args.db, cohort=args.cohort,
