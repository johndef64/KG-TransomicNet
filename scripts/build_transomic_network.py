...
==========================
Build a transomic property-graph JSON for one TCGA sample by joining its
filtered omic features with the surrounding subgraph of the PheKnowLator KG.

Usage examples
--------------
    # Default DB, auto-pick a BRCA sample with complete omics
    python scripts/build_transomic_network.py --cohort TCGA-BRCA

    # Explicit sample + biological predicates + per-omic thresholds
    python scripts/build_transomic_network.py \\
        --db PKT_main --cohort TCGA-BRCA \\
        --sample-id TCGA-BH-A18U-01A \\
        --use-default-thresholds --use-biological-predicates

    # Custom server / credentials
    python scripts/build_transomic_network.py \\
        --host http://my-server:8529 --user me --password pass \\
        --db MyDB --cohort TCGA-BRCA
...
--------------------
  ...
  --sample-id ID           Specific sample _key (auto-pick if omitted).
  --use-default-thresholds Apply DEFAULT_VALUE_THRESHOLDS per-omic preset.
  --use-biological-predicates / --use-molecular-predicates / --use-disease-predicates
                           Apply one of the three predicate presets.
  --output PATH            Where to write the JSON (auto-named if omitted).
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--db", "--db-name", dest="db", default="PKT_main",
                        help="ArangoDB database name (default: PKT_main).")
    ...
    parser.add_argument("--user", default=arangodb_utils.arangodb_user,
                        help=f"ArangoDB user (default: {arangodb_utils.arangodb_user}).")
    parser.add_argument("--password", default=arangodb_utils.arangodb_password,
...
def main():
    ...
    # Honour --host/--user/--password overrides at runtime by patching
    # the arangodb_utils module globals that setup_arangodb_connection reads.
    import arangodb_utils
