...
========================
Descriptive analysis of the per-sample transomic property-graph JSON files
produced by build_transomic_network.py.

This script reads local JSON files only — no ArangoDB connection required.

Outputs (printed and optionally written to disk):
- Node / edge counts (overall and per omic layer)
- Number of nodes successfully linked back to a KG node
- Value distribution per omic (min / max / mean / count)
- Predicate-label frequency table
- Neighbour bioentity-type distribution
- Optional top-N |value| features per omic

Usage examples
--------------
    # Analyze a single graph JSON
    python scripts/analyze_kg_transomics.py \\
        --file transomic-networks/transomic_graph_TCGA-BRCA_TCGA-BH-A18U-01A.json

    # Pick the first JSON in a folder whose filename contains a substring
    python scripts/analyze_kg_transomics.py \\
        --folder transomic-networks --sample TCGA-BH-A18U

    # Print also the top-10 features per omic by |value|
    python scripts/analyze_kg_transomics.py --file <path> --print-top 10

    # Persist the JSON summary report (machine-readable)
    python scripts/analyze_kg_transomics.py --file <path> --output report.json

Library use
-----------
    from analyze_kg_transomics import load_graph, analyze_transomic_graph
    g = load_graph("transomic-networks/transomic_graph_<cohort>_<sample>.json")
...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="Analyze transomic property-graph JSON files (no DB required).",
	)
	parser.add_argument("--file", help="Path to a single graph JSON to analyze.")
	parser.add_argument("--folder", default=os.path.join("..", "transomic-networks"),
						help="Folder containing graph JSON files (used if --file is not provided).")
	...
	parser.add_argument("--print-top", type=int, default=0,
						help="If >0, print top-N features per omic layer (by |value|).")
	parser.add_argument("--output", default=None,
						help="If provided, also write the JSON summary report to this path.")
	return parser.parse_args()


def _resolve_target_file(args) -> Optional[str]:
	"""Return the path of the single JSON to analyze, honouring --file/--folder/--sample."""
	if args.file:
	...
	if not files:
		print("No graph files found in", folder)
		return None
...
def main():
	args = parse_args()
	path = _resolve_target_file(args)
	if path is None:
		return
