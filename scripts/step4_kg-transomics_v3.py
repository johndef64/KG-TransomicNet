...
1: [Knowledge Graph in PKT_test10000:]
- collection: nodes
...
2: [Omics Data Collections in PKT_test10000: ]

| Collection      | Scope / contenuto principale                                          | Chiave e campi chiave                                                                                                                                                                                | Uso tipico in query                                                                     |
| --------------- | --------------------------------------------------------------------- | -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------- |
| PROJECTS        | Metadata TCGA di progetto (es. TCGA-BRCA, TCGA-LUAD, …)               | `_key`= project_id TCGA                                                                                                                                                                              | Navigare coorti, descrizioni e mapping project ↔ studi.                                 |
...
| GENE_EXPRESSION | Campioni con vettori di espressione per campione +  indice            | `data_type`="gene_expression_vector": `_key`=sample_id; `values_tpm`, `expression_index_ref`<br>`data_type`="gene_expression_index": `_key`=expr_index_TCGA-\<STUDY\>; `gene_mappings[]`   | Costruire matrici gene × sample; recuperare ordine geni per interpretare vettori.       |
| CNV             | Campioni con vettori CNV per campione +  indice                       | `data_type`="cnv_vector": `_key`=sample_id; `values_copy_number`, `cnv_index_ref`<br>`data_type`="cnv_index": `_key`=cnv_index_TCGA-\<STUDY\>; `gene_mappings[]`                           | Analisi CNV per coorte/case; ottenere mapping gene→posizione nel vettore.               |
| METHYLATION     | Campioni con vettori di metilazione per campione +  indice            | `data_type`="methylation_vector": `_key`=sample_id; `values_beta`, `methylation_index_ref`<br>`data_type`="methylation_index": `_key`=methylation_index_TCGA-\<STUDY\>; `probe_mappings[]` | Analisi metilazione per coorte/case; mapping probe→posizione nel vettore.               |
| MIRNA           | Campioni con vettori di espressione miRNA per campione +  indice      | `data_type`="mirna_vector": `_key`=sample_id; `values_expression`, `mirna_index_ref`<br>`data_type`="mirna_index": `_key`=mirna_index_TCGA-\<STUDY\>; `mirna_mappings[]`                   | Analisi miRNA e integrazione multi-omica; mapping miRNA→posizione nel vettore.          |
| PROTEIN         | Campioni con vettori proteomici per campione +  indice                | `data_type`="protein_vector": `_key`=sample_id; `values_abundance`, `protein_index_ref`<br>`data_type`="protein_index": `_key`=protein_index_TCGA-\<STUDY\>; `protein_mappings[]`          | Layer proteomico per modelli multi-omici; mapping peptide→posizione nel vettore.        |

### Note sulla struttura unificata:

Ogni collection omica (GENE_EXPRESSION, CNV, METHYLATION, MIRNA, PROTEIN) contiene **due tipi di documenti** distinti identificati dal campo `data_type`:

1. **Documenti vettore** (`*_vector`): Contengono i dati quantitativi per campione
   - `_key` = sample_id (es. `TCGA-D8-A146-01A`)
   - Array di valori (es. `values_tpm`, `values_copy_number`, `values_beta`, ecc.)
   - Reference all'indice corrispondente (es. `expression_index_ref`)

2. **Documenti indice** (`*_index`): Contengono il mapping posizione→feature
   - `_key` = index_id (es. `expr_index_TCGA-BRCA`)
   - Array di mappings dove `position` corrisponde all'indice nel vettore dei valori
   - Reference ai geni/probe/miRNA nella collection di riferimento

### Esempio di join per ottenere espressione di un gene specifico:
"""

JOIN_TEST_AQL = """
// Recupera TPM di TP53 per tutti i campioni BRCA
...
FOR sample IN GENE_EXPRESSION
  ...
  RETURN {
    ...
    tpm: sample.values_tpm[gene_data.position],
    hgnc_symbol: gene_data.hgnc_symbol,
    entrez_id: gene_data.entrez_id
  }
"""

def get_data_structure_info(collection_name, data_type=None):
    """
    get dict atrigutes kyes and values  from on entry in collection_name
    """
    filer_string = f'FILTER doc.data_type == "{data_type}"' if data_type else ""
    aql_query = f"""
    FOR doc IN {collection_name}
      // example filter to get a specific document type
      {filer_string}
      LIMIT 1
      RETURN doc
    """
...
def test_query(query_aql, timer  = True):
    import time
    time_start = time.time()
    result = db_connection.aql.execute(query_aql)
    for doc in result:
        if timer: print(f"Query executed in {time.time() - time_start:.2f} seconds")
        return doc
    
# for data structure see "db_structure_omics_data.md"
test_query(JOIN_TEST_AQL)

get_data_structure_info("GENE_EXPRESSION", data_type="gene_expression_index")
...
#- **Risoluzione di un gene KG → posizione indice (coorte)** (nuova struttura unificata): 
AQL = """
LET idx = FIRST(
    FOR doc IN GENE_EXPRESSION
        FILTER doc.data_type == "gene_expression_index"
        FILTER doc._key == @indexKey
        RETURN doc
)
LET pos = FIRST(
    FOR m IN idx.gene_mappings
        FILTER m.entrez_id == @entrezId OR m.hgnc_symbol == @hgnc
        RETURN m.position
)
RETURN pos
"""
...
#data la struttura di questi dati, devi sviluppare un sistema AQL python che permetta il recupero dei valori quantitativi dagli array dei samples per la variabile inserita esempio entrez_id
...
from omics_query_utils import (
    ...
    get_gene_vector,
    get_genes_vector_json,
    get_sample_list_intersection,
    OMIC_CONFIG
)

#%% Test: Resolve gene position (nuova struttura unificata)
pos = resolve_gene_position(db_connection, "TCGA-BRCA", entrez_id="7157")
print(f"TP53 position in expression vector: {pos}")

#%%
cnv_pos = resolve_cnv_position(db_connection, "TCGA-BRCA", entrez_id="7157")
print(f"TP53 position in CNV vector: {cnv_pos}")
print("-----")
# check actual value loading "data\arangodb_collections\TCGA-BRCA\cnv_index.json"
import json
with open("../data/arangodb_collections/TCGA-BRCA/cnv_index.json", "r") as f:
    cnv_index_data = json.load(f)
for mapping in cnv_index_data['gene_mappings']:
    if mapping['entrez_id'] == "7157":
        print(f"TP53 CNV mapping: {mapping}")

#%%
# check get_genes_vector_json for CNV index
vector = get_genes_vector_json(cnv_index_data, id_type="position")
print(f"Vector length: {len(vector)}. First 5 genes in CNV index (Ensembl IDs): {vector[:5]}")
#%%   -------

# get_gene_vector con nuova struttura unificata
get_gene_vector(db_connection, "TCGA-BRCA",
                id_type="peptide_target", 
                omic_type="protein")
#%%
get_data_structure_info("GENE_EXPRESSION", data_type="gene_expression_index")

#%%
cnv_index_data['gene_mappings']
#%% Test: Get TP53 expression across all BRCA samples
gene_expr = get_gene_expression_by_gene(
    db_connection, 
    cohort="TCGA-BRCA",
    hgnc_symbol="TP53",
    value_type="tpm"
)
print(f"Found expression for {len(gene_expr)} samples")
for r in gene_expr[:5]:
...
GENE = "TP53"
gene_data = get_gene_info(db_connection, hgnc_symbol=GENE)

gene_multiomics = get_multiomics_by_gene(
    db_connection,
    cohort="TCGA-BRCA",
    hgnc_symbol= gene_data['hgnc_symbol'],
    entrez_id=gene_data['entrez_id']
...
print(f"  Expression samples: {len(gene_multiomics['expression'])}")
print(f"  CNV samples: {len(gene_multiomics['cnv'])}")
print(f"  Methylation samples: {len(gene_multiomics['methylation']['samples'])}")
# print(f"  Methylation probes: {len(gene_multiomics['methylation'].get('probes', []))}")
print(f"  Protein samples: {len(gene_multiomics['protein'])}")
#%%
from omics_query_utils import get_sample_list_intersection
common_samples = get_sample_list_intersection(gene_multiomics) 
print(f"  Common samples across all omics: {len(common_samples)}")
list(common_samples)[:3]
#%%
gene_multiomics.keys()
#%%
gene_multiomics['methylation']
